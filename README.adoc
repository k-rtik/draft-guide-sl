...
You will learn how to secure and test a simple REST service with SocialLogin.
The service responds to a `GET` request with a Plain Text response that looks like this:
...
This feature allows you to easily secure your service using social media OAuth2 and OpenID Connect providers.

=== Prerequisites

Before you can run/build the application, you will need to get the required certificates.
...
You also need to create certificates that your service will provide for HTTPS connections. These public/private keys will need to be added to the keystore.
...
Lastly, in order to use the new key store and trust store, copy these files into `src/main/liberty/resources/security`
