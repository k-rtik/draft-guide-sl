...
The [hotspot=sslConfig]`authServiceSslRef` configuration element which references `defaultKeyStore` as the key store
and `socialLoginKeyStore` as the trust store will be provided to social media login configurations.
...
in the prerequisites section. The `sslRef` attribute is set to the [hotspot=sslConfig]`ssl` configuration from the previous section.
