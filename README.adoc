...
:page-description: Learn how to secure a REST service with the Open Liberty Social Login feature.
...
Learn how to secure a REST service with the Open Liberty Social Login feature.
...
== Adding social media login configurations

=== Adding Github login configuration

First, you will configure Github login using the `githubLogin` configuration element.

Create a [hotspot=githubLogin]`githubLogin` configuration using the `clientId` and `clientSecret` for your Github application
that you created in the prerequisites section. Add the ID of the [hotspot=sslConfig]`ssl` configuration that you created in the previous section
to the `sslRef` attribute of `githubLogin`.

When the server updates its configuration, when you access https://localhost:9080/api/hello[the endpoint for Hello service],
you should be redirected to the Github login page for your application.

=== Adding Amazon login configuration

Next, you will configure Amazon login using a custom `oauth2Login` configuration element.

Create a [hotspot=amazonLogin]`oauth2Login` configuration using the `clientId` and `clientSecret` for your Amazon application
that you created in the prerequisites section. Add the following attributes to your [hotspot=amazonLogin]`oauth2Login` configuration element:
...
In addition to these attributes, add the ID of the [hotspot=sslConfig]`ssl` configuration that you created in the previous section
to the `sslRef` attribute of [hotspot=amazonLogin]`oauth2Login`.
...
Lastly, you will configure Google login using a custom `oidcLogin` configuration element.

Create a [hotspot=googleOIDCLogin]`oidcLogin` configuration element using the `clientId` and `clientSecret` for your Google application
that you created in the prerequisites section. Add the following attributes to your [hotspot=googleOIDCLogin]`oidcLogin` configuration element:
...
In addition to these attributes, add the ID of the [hotspot=sslConfig]`ssl` configuration that you created in the previous section
to the `sslRef` attribute of [hotspot=googleOIDCLogin]`oidcLogin`.
...
The test code needs to know some information about the application to make requests.
The server port and the application context root are key, and are dictated by the server configuration.
While this information can be hardcoded, it is better to specify it in a single place like the Maven [hotspot=defaultHttpPort hotspot=defaultHttpsPort hotspot=appContextRoot file=1]`pom.xml` file.
Refer to the [hotspot file=1]`pom.xml` file to see how the application information such as the [hotspot=defaultHttpPort file=1]`default.http.port`, [hotspot=defaultHttpsPort file=1]`default.https.port` and [hotspot=appContextRoot file=1]`app.context.root` elements are provided in the file.
...
You can do this by looking for the string [hotspot=assertredirect file=0]`"Social Media Selection Form"` in the response string.
...
You secured and tested a simple REST service in Open Liberty by using the Social Login feature.
